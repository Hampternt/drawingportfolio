...
Two rules sit under everything here: **never make the user wait for work that can finish
after the response**, and **never move the page once it has painted**. The repo already
follows both; this section is the written form so a new section inherits them.

### Load order

Every page's head, in this order: `preconnect` and `dns-prefetch` to the image host,
the one stylesheet, `prefetch` for the three sections a visitor goes to next, then the
scripts — all `defer`, in dependency order, all self-hosted. HTMX ships from
`/static/htmx.min.js` rather than a CDN precisely to avoid a third-party DNS lookup and
TLS handshake. One global stylesheet, no build step, no framework.

Fonts preload **per page, not globally**: only the faces above the fold on that page
(`archivo-800` and `space-grotesk-400` on the feed). A page never pays for a face it
does not show.

At the edge: gzip HTML, CSS, JS, JSON and SVG; `expires 1y` plus
`Cache-Control: public, immutable` on `/static/` so a return visitor never touches the
network for it (filenames carry no hash, so renaming is the cache bust); `ssl_session_cache`
and OCSP stapling so a returning visitor skips the handshake and the browser skips its own
trip to the CA.

### Perceived speed

- **Boosted navigation.** `hx-boost="true"` on `<body>` swaps content, so the header, nav
  and palette never blank out and a section change reads as a state change.
- **The palette beats the page.** `Ctrl` `K` and three letters is faster than any render —
  the reason the palette is the primary navigation, not a shortcut.
- **Reserve the space.** `width` and `height` on every `<img>`, both or neither. Cards hold
  their size before the bytes land; nothing reflows under a reader mid-scroll.
- **Prioritise one image.** First card `loading="eager" fetchpriority="high"`, everything
  below it `loading="lazy"`.
- **Smallest format the browser accepts.** `<picture>` offering AVIF, then WebP, then the
  original; a missing variant is an empty URL that falls through rather than breaking.
- **Edit in place.** A mutation returns the one card it changed and swaps it `outerHTML`.
- **Respect `prefers-reduced-motion`.** Every transition and keyframe has a reduce branch.

### Respond first, finish after

The upload path is the model. The browser converts the image to WebP on a canvas before it
leaves the client, so the server never encodes a format the user already has. The request
stores the file, inserts the row and returns the finished card in two or three seconds —
the user can navigate away the moment they click. AVIF encodes in a detached task nobody
awaits and backfills the URL on the existing row; its failures are logged, never surfaced.
That moved a 39-second upload to three. If a step does not change what the user sees next,
it does not belong in the request.

### The cost of keeping the page alive

`hx-boost` keeps one document alive across navigations, which has four consequences every
script must handle:

1. **Bind twice.** `DOMContentLoaded` fires once per session, so every initialiser also
   binds on `htmx:afterSwap`, guarded by an existence check so a boosted return visit does
   not stack duplicates.
2. **Guard the swap target.** Act only when `e.target === document.body`; otherwise every
   fragment swap re-runs page setup against a half-updated DOM.
3. **Attributes are not swapped.** Boosting replaces body *children*, so re-derive theme
   classes from a marker element in the incoming content rather than trusting a class on
   `<body>`.
4. **Nothing unloads.** Camera streams, timers and SSE connections outlive the navigation —
   stop them on swap yourself.
