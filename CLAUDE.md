...
The repo is a cargo workspace, but the root `Cargo.toml` is *also* a package — so bare `cargo test` runs the current package only and silently skips `drinkinggame`'s 177 tests (**216 of 393 run**). Always pass `--workspace`, or just run `./scripts/verify.sh`, which does. `cargo run -p drinkinggame` serves the drinking game standalone on `:3001` (no portfolio, no nginx).
...
- `src/routes/feed.rs` — `GET /artportfolio` (art feed), `GET /artportfolio/htmx/posts?page=N&q=&last_month=&tags=&collection=&vis=` (HTMX month sections), `GET /artportfolio/api/posts?page=N&q=&tags=&collection=&vis=` (JSON API), `GET /artportfolio/{id}` (single-post permalink). All four share `PageQuery::filter()`, which builds one `PostFilter` from `q` (caption), `tags` (comma-separated), `collection` (slug) and `vis` (comma-separated subset of `public,unlisted,hidden`, admin-only — silently dropped for a non-admin or previewing viewer); `db::get_posts_page()` and `db::count_posts()` both take that same `PostFilter`, so the page head's total and the grid always agree. Posts are grouped into `MonthGroup`s **in the handler** (keyed on `created_at[..7]`), each month rendering its own `columns` block — `last_month` suppresses the duplicate divider when a month spans a page boundary. Search does **not** use `hx-push-url`: that pushes the fragment URL, so `htmx_posts` returns an `HX-Push-Url: /artportfolio?q=…` header on page 0 only. The filter rail (`templates/artportfolio/partials/filter_rail.html`, `rail_filters.html`, `rail_collections.html`) renders collections and tags as toggle links built by `collection_rail_links`/`tag_rail_links` — each link's URL already carries the *full* next filter state (not just its own toggle), built from the current `PostFilter` via `filter_url`/`page_url`, so composing filters (a collection click then a tag click) needs no client-side merging — plus, admin-only, the visibility trio and a `+` new-collection input. A rail click still only swaps `#feed`, so `htmx_posts` re-renders `rail_filters.html` (collections/tags/vis, shared by `{% include %}` with the full-page path) as an `#art-rail-filters` out-of-band block on every page-0 htmx response, rebuilt from the just-applied filter — this is what keeps `is-active`/`is-checked` states and counts from freezing after the first click. An `#art-rail-state` hidden input (`active_collection`/`active_tags`/`active_vis`/`preview`) exists for one consumer only: the search input's own `hx-include`, since typing a search is the one rail control whose own request does not carry the rest of the filter.
...
- **Tests:** `src/db.rs` holds the db-layer tests (nutrition CRUD, slots, targets, ranges, recipes, weights) and `src/routes/nutrition.rs` has unit tests for the ring/rail math and streak logic — 393 tests across the workspace.
