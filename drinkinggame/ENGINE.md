...
| **Standing rules** | `lc_cards::RuleDef`, `LcPlayer::rules`, `rule_text()` | A social instruction pinned to a seat for N rounds — the engine remembers, announces and expires it; the table enforces it. Two sources, one stored form: a rule card's own `CardEffect::Rule`, or a challenge card's `Penalty::Rule` carried by its loser. `PublicSeat::rules` makes it visible, which it was not before. |
